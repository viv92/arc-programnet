...
# class for Example Decoder Embedding
class Example_Decoder_Embedding(nn.Module):
    def __init__(self, example_dim, d_model, device):
        super().__init__()
        self.x_emb = nn.Linear(example_dim, d_model, bias=False)
        self.device = device
    def forward(self, x): # x.shape: [batch_size, max_seq_len, example_dim]
        x_emb = self.x_emb(x) # x_emb.shape: [batch_size, max_seq_len, d_model]
        return x_emb 
    
# class for Example Encoder Embedding
class Example_Encoder_Embedding(nn.Module):
...
# class implementing Example Decoder Transformer 
class Example_Decoder_Transformer(nn.Module):
    def __init__(self, embedder, decoder, d_model, out_dim, out_seq_len):
        super().__init__()
        self.embedder = embedder 
        self.decoder = decoder
        self.out_proj = nn.Linear(d_model, out_dim, bias=False)
    ...
    def forward(self, x): # x.shape: [batch_size, max_seq_len, x_dim]
        x_emb = self.embedder(x) # x_emb.shape: [batch_size, max_seq_len, d_model]
        out = self.decoder(x_emb, mask=None) # encoder_out.shape: [batch_size, max_seq_len, d_model]
        final_out = self.out_proj(out) # final_out.shape: [batch_size, max_seq_len, out_dim]
        final_out = final_out[:, :self.out_seq_len]
        return final_out
    
# class implementing Example Encoder Transformer 
class Example_Encoder_Transformer(nn.Module):
    def __init__(self, embedder, encoder, d_model, out_dim, out_seq_len):
        super().__init__()
        self.embedder = embedder 
        self.encoder = encoder
        self.out_proj = nn.Linear(d_model, out_dim, bias=False)
    ...
    def forward(self, x): # x.shape: [batch_size, max_seq_len, x_dim]
        ...
        encoder_out = self.encoder(x_emb, mask=None) # encoder_out.shape: [batch_size, max_seq_len, d_model]
        final_out = self.out_proj(encoder_out) # final_out.shape: [batch_size, max_seq_len, out_dim]
        final_out = final_out[:, :self.out_seq_len]
        return final_out


# class implementing Example Transformer
class Example_Transformer(nn.Module):
    def __init__(self, encoder_transformer, decoder_transformer, device):
        super().__init__()
        self.encoder = encoder_transformer 
        self.decoder = decoder_transformer 
        self.device = device 
    ...
    def forward(self, items): 
        batch_size, num_examples = len(items), items[0]['examples'].shape[0]
        examples_batch = []
        for i in range(batch_size):
            examples_batch.append(items[i]['examples'])
        examples_batch = torch.stack(examples_batch) # [b, num_examples, seqlen * 2]
        examples_batch = examples_batch.flatten(start_dim=0, end_dim=1).to(self.device) # [b * num_examples, seqlen * 2]
        examples_batch = self.encoder(examples_batch) # [b * num_examples, example_emb_seqlen, example_emb_dim]
        example_emb_seqlen, example_emb_dim = examples_batch.shape[-2], examples_batch.shape[-1]
        examples_batch = examples_batch.view(batch_size, num_examples, example_emb_seqlen, example_emb_dim)
        examples_batch = examples_batch.flatten(start_dim=1, end_dim=2) # [b, num_examples * example_emb_seqlen, example_emb_dim]
        example_embs = self.decoder(examples_batch) # [b, seqlen, task_emb_dim]
        return example_embs 
    

# caller function to instantiate example decoder transformer
def init_example_decoder_transformer(in_dim, max_seq_len, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, out_dim, out_seq_len, device):
    rope = RotaryPositionalEmbeddings(d_model, max_seq_len ** 2, device)
    attn = FSQ_MultiHeadAttention(n_heads, d_model, d_k, d_v, rope, dropout) # multi head attention block
    ff = FeedForward(d_model, d_ff, dropout) # feed forward block for each encoder / decoder block
    embedder = Example_Decoder_Embedding(in_dim, d_model, device) # embedder block to obtain sequence of embeddings from sequence of input tokens
    decoder_layer = FSQ_DecoderLayer(deepcopy(attn), deepcopy(ff), d_model, dropout) # single encoder layer
    decoder = FSQ_Decoder(decoder_layer, n_layers, d_model) # encoder = stacked encoder layers
    model = Example_Decoder_Transformer(embedder, decoder, d_model, out_dim, out_seq_len) # a non-causal encoder-only transfomer
    # initialize params - Xavier initialization
    for p in model.parameters():
        if p.dim() > 1:
            nn.init.xavier_uniform_(p)
...
# caller function to instantiate example encoder transformer
def init_example_encoder_transformer(vocab_size, max_seq_len, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, example_dim, out_seq_len, device):
    rope = RotaryPositionalEmbeddings(d_model, max_seq_len ** 2, device)
    attn = FSQ_MultiHeadAttention(n_heads, d_model, d_k, d_v, rope, dropout) # multi head attention block
    ff = FeedForward(d_model, d_ff, dropout) # feed forward block for each encoder / decoder block
    embedder = Example_Encoder_Embedding(vocab_size, d_model, device) # embedder block to obtain sequence of embeddings from sequence of input tokens
    ...
    model = Example_Encoder_Transformer(embedder, encoder, d_model, example_dim, out_seq_len) # a non-causal encoder-only transfomer
    # initialize params - Xavier initialization
    for p in model.parameters():
        if p.dim() > 1:
            nn.init.xavier_uniform_(p)
